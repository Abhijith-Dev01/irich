...
from serializers import business_detailsSerializer, businessSerializer, categorySerializer, \
    EmployeeSerializer, dealSerializer, paymentSerializer, rewardSerializer, transSerializer, usersSerializer,walletpaymentSerializer,UserRegistrationSerializer,UserSerializer,UserTokenObtainPairSerializer,UserdetailsSerializer
    ...
    for payment in Payment:
        for dicts in wallet_user_ids:
            if dicts['user_id'] == payment.user_id:
                ...
                # from_value.save()
                percentage=int(payment.irich.irich)/100
                details.append({
...
class paysection(APIView):
    ...
    def post(self, request):
        ...
        if Serializer.is_valid():
            ...
            wallet_user_ids = wallet.objects.all().values('user_id','irich_bonus')
            print(wallet_user_ids)
            Payment_details = payments.objects.all().select_related('irich', 'user').only('irich__business_name', 'irich__irich',
                                                                        'user__username')
            
            client = razorpay.Client(auth=("rzp_test_fNN7BgVDDfS2Vf", "iQrrBDw9pZjTuy5mQ0tS2mFr"))
            payment = client.order.create({'amount': int(amount) * 100, 'currency': 'INR',
                                    'payment_capture': '1'})
                                  
            Serializer.user_id = user_id             
            Serializer.payment = payment
            Serializer.save()
...
class walletpaysection(APIView):
    ...
    def post(self, request):
        ...
        print(data.get("user"))
        wallet_user_ids = wallet.objects.all().values('user_id','irich_bonus')
        Payment = payments.objects.all().select_related('irich', 'user', 'irich_walletamount' ).only('irich__business_name', 'irich__irich',
                                                                      'user__username','irich_walletamount__irich_bonus')
        for payment in Payment:
            ...
            for dicts in wallet_user_ids:
                if dicts['user_id'] == payment.user_id:
                    amount=request.POST.get('amount')
                    user=request.POST.get('user')
                    irich_walletamount=request.POST.get('irich_walletamount')
                    print(irich_walletamount)
                    print(user)
                    bonus = (int(dicts['irich_bonus']) - int(amount))
                    print(bonus)
                    from_value = wallet.objects.get(user_id=user_id)
                    from_value.bonus = bonus
                    from_value.save()
        if Serializer.is_valid():
            Serializer.save()
            return JsonResponse(Serializer.data)
...
class ViewProfile(APIView):
    ...
    def get(self, request, format=None):
        ...
        return Response({'profile details':serializer.data})        
